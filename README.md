...
GOAL: to find up to "nc" unique controls within a certain "threshold" "distance" of each case, based on principal components of a standardised genotype matrix of the all cases and controls. 
Remove cases that cannot be adequately matched. 

## INPUTS: 
Requires:
* PCA eigenvalues 
* PCA scores (e.g. plink.eigenvec, or output of e.g. Florian Prive bigspr pca (https://privefl.github.io/bigsnpr/articles/bedpca.html)
...
* Scree plot
* List of cases and controls kept and remove
...
* Two algorithums are used.
  * Localbestworst:
    * Ignoring all distances greater than the threshold, for each case the nearest-neighour control is identified
    * For each case with fewer than min controls matched:
      * The case with the furthest near-neighbour control is assigned that control. 
  ...
  * Min-cost-max-flow
    * For remaining set of cases with 0 < matches  <min_nc , and the set of controls that were assigned to these cases, perform a min-cost max-flow algorithm to maximise the number of cases with min_nc matched controls.
    * This is especially important for 'rescuing' rarer ancestry cases where controls in the previous step might have been divided between different cases without any reaching the minimum requirement. 
      

## COMMENTS
* Results are sensitive to the method chosen, especially weighting and the number of controls sought.   
...
Updated 2024-5 by GT Doctor UCL Centre for Genetics and Genomics https://github.com/gtdoctor
